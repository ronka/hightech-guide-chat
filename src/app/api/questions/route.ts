...
export async function GET() {
  try {
    ...
    for (const fileName of mdxFiles) {
      ...
      try {
        ...
        const item: QuestionListItem = {
          ...
          companies: Array.isArray((frontmatter as any).companies)
            ? (frontmatter as any).companies
            : undefined,
        };
    ...
    return NextResponse.json(
      {
        code: "questions_read_failed",
        error: "Failed to read questions",
        hint: "This is a transient server error; retry the request.",
      },
      { status: 500 },
    );
  }
